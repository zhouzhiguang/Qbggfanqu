...
package com.qbgg.cenglaicengqu.common.ui.viewpager;
...
import com.qbgg.cenglaicengqu.main.reminder.ReminderSettings;
...
public class PagerSlidingTabStrip extends HorizontalScrollView implements OnPageChangeListener {
    ...
    private int underlineColor = ContextCompat.getColor(getContext(),R.color.skin_page_tab_underline_color);// 0xFFD9D9D9;
    ...
    private void setChooseTabViewTextColor(int position) {
        ...
        int size= (int) getResources().getDimension(R.dimen.dimen_55px);
        for (int i = 0; i < childCount; ++i) {
            tabView = (LinearLayout) tabsContainer.getChildAt(i);
            textView = (TextView) tabView.findViewById(R.id.tab_title_label);
            textView.setTextSize(size/value);
            if (i == position) {
                textView.setTextColor(ContextCompat.getColor(getContext(),checkedTextColor));
            } else {
                textView.setTextColor(ContextCompat.getColor(getContext(),unCheckedTextColor));
            }
        }
    }
