...
public class AnFQNumEditText extends RelativeLayout {
    ...
    private TextView tvNum, tvNum_tips;//字数显示TextView
    ...
    public AnFQNumEditText(Context context, AttributeSet attrs) {
        ...
        tvNum_tips = (TextView) findViewById(R.id.tvNum_tips);
        vLine = findViewById(R.id.vLine);
        tips = getResources().getString(R.string.tvnum_tips);
    ...
    public AnFQNumEditText show() {
        tvNum_tips.setText(String.format(tips, "0"));
        if (TYPES.equals(SINGULAR)) {//类型1
            tvNum.setText(String.valueOf(MinNum));
        } else if (TYPES.equals(PERCENTAGE)) {//类型2
            tvNum.setText(0 + "/" + MinNum);
           // tvNum_tips.setText(String.format(tips,0) );
    ...
    private void setLeftCount() {
        ...
        if (TYPES.equals(SINGULAR)) {//类型1
            if (inputcount > MinNum) {
                tvNum.setText(String.valueOf((MinNum)));
            } else {
                tvNum.setText(String.valueOf((MinNum - inputcount)));
            }
        } else if (TYPES.equals(PERCENTAGE)) {//类型2
            if (inputcount <= MinNum) {
                tvNum_tips.setText(String.format(tips, String.valueOf(MinNum - getInputCount())));
                tvNum.setText(MinNum - (MinNum - getInputCount()) + "/" + MinNum);
            }else {
                tvNum.setText(String.valueOf(MinNum + "/" + MinNum));
                tvNum_tips.setText(String.format(tips,String.valueOf("0") ));
            }
    ...
    public void setminLength(int inportMinLength) {
        this.MinNum = inportMinLength;
    }

    public String getInputText() {
        if (etContent !=null){
            Editable editable=  etContent.getText();
            return editable==null ? null:editable.toString().toString();
        }else {
            return null;
        }
    }
